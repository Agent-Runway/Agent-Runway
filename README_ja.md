...
| 🚫 Assertion-language gate | 多言語パターンが `turn_end_gate` / `completion_gate` の summary にある曖昧な表現を拒否します |
| 🔬 Counterexample | `record_counterexample_check` -> 仮説 + 反証確認 + 残余リスク |
| 📝 Decision records | `record_decision_record` -> 選択 + 却下した代替案 + 再オープン条件 |
| 🔐 Authorization | 不可逆な操作には、記録され、なお有効なユーザー承認が必要。すでに承認されたコマンドは再確認しない |
...
| ⛔ Stop enforcement | 物理ブロックは Claude Code の `Stop` hook が設定済みの場合のみ。MCP/OpenCode/Pi/Codex/Cursor には Stop hook parity はありません |
| ⚠️ Risk-event interception | Claude hooks は危険な shell command と保護パス読み取りを確認/拒否できます。OpenCode bridge は有効化時に fail-closed できます。Pi CLI は extension-only です |
| ⚖️ Value Gate | 高 impact・検証可能・低 expansion の場合だけ続行 |
| 🧠 Project Learning Ledger | プロジェクトの `.agent-runway` に置くローカル JSONL。pitfalls、runbooks、preferences、invariants を記録し、advisory のみで evidence や authorization にはならない |
...
| 権限を越える | durable な承認なしに deploy、push、外部呼び出しをする | 不可逆操作の前に authorization record がまだ有効か確認する。すでに承認されたコマンドは再確認せず、範囲外のコマンドには引き続き承認が必要 |
...
receipt に支えられる governance record には、実際に捕捉された receipt_ids が必要です。Codex や Cursor、あるいは自動 tool-receipt capture のない指示型 MCP パスでは、空の `receipt_ids` リストは capability gap のサインであり、有効な record ではありません。直接のローカル evidence は別途明示するか、host bridge を直してください。ID をでっち上げてはいけません。
...
OpenCode はデフォルトで native MCP configuration を使います。このリポジトリには `.opencode/plugins/agent-runway.js` として実際の bridge 実装が含まれていますが、OpenCode は skill directory 内の plugin を自動検出しません。ローカル plugin を自動ロードするのは、project の `.opencode/plugins/`、ユーザーの `~/.config/opencode/plugins/`、または Windows の `%USERPROFILE%\.config\opencode\plugins\` だけです。OpenCode の tool events を自動で receipt ledger に転送したい場合は、これらの OpenCode plugin 配置先のどれかに `shim` または `symlink` を置き、skill plugin を再エクスポートさせたうえで `ILH_OPENCODE_BRIDGE=1` を設定してください。これにより receipt capture は改善され、OpenCode が `exit`、`exitCode`、`exit_code` を返したときの shell exit code も引き継げます。ただし Claude 風の Stop-hook 対等性はありません。

Pi CLI support は意図的に狭い範囲です。`python scripts/generate_host_config.py --host pi-cli --agent-runway-dir <agent-runway-dir>` は extension-only note を出すだけで、native MCP configuration は出力しません。検証済みの interception path は、`pi.on("tool_call", ...)` で `{ block: true, reason: "..." }` を返す Pi extension です。`scripts/fixtures/pi_block_extension.js` を参照してください。
...
- **summary 内の断定表現の拒否。** `turn_end_gate` は `work_summary` を、`completion_gate` は `completion_summary` を、"should work", "probably", "I believe" などを含む多言語パターンで検証します。
- 検証済みの slice stop が、まだ残っている次の高価値キャンペーン、新しい alpha source、テンプレート再設計を assumption、risk、unverified item に隠してはいけません。実際の残作業として名前を付け、続行するか、権限や情報が本当に足りないときだけ合法な soft stop を使ってください。
...
1. 前提条件: Python 3.11+
2. Agent-Runway を、この CLI/host が読み込む skills directory に clone: git clone https://github.com/Agent-Runway/Agent-Runway <cli-skills-dir>/agent-runway
3. 依存をインストール: pip install mcp
4. 設定を生成: python scripts/generate_host_config.py --agent-runway-dir <agent-runway-dir>
   host が Claude Code 以外なら、--host opencode、--host codex、--host cursor、または --host pi-cli だけを追加する
5. 出力された JSON を、現在の host に合った正しい設定先へ merge:
   - Claude Code -> Claude Code workspace または host config が実際に使う .claude/settings.json
...
9. 検証: python scripts/quick_validate.py <agent-runway-dir>
```

### 手動インストール

**前提条件:** Python 3.11+。Agent-Runway を CLI または AI host が読み込む skills directory に clone してください: `git clone https://github.com/Agent-Runway/Agent-Runway <cli-skills-dir>/agent-runway`。この clone された directory が `<agent-runway-dir>` で、`SKILL.md`、`scripts/`、`mcp/` を含みます。host config file の directory ではありません。
...
`<agent-runway-dir>` から次のデフォルトコマンドで設定 JSON を生成します。`--host` を指定しない場合は Claude Code 向けです。

```bash
python scripts/generate_host_config.py --agent-runway-dir <agent-runway-dir>
```

OpenCode、Codex、Cursor、Pi CLI では同じコマンドを使い、`--agent-runway-dir` の前に `--host opencode`、`--host codex`、`--host cursor`、または `--host pi-cli` を追加します。

Pi CLI の出力は extension-only capability note であり、native MCP installer ではありません。

#### 2. 設定を対応するファイルへコピーする

**Claude Code:** 出力された JSON を、Claude Code workspace または host config が実際に使う `.claude/settings.json` に merge します。
...
**runtime ファイルの既定位置:**
- データベース: アクティブなプロジェクトディレクトリ配下の `.agent-runway/state.db`。複数プロジェクトで skill のインストールディレクトリを共有デフォルト DB として使わないでください
...
**Windows メモ:**
- `--agent-runway-dir` には絶対パスを使ってください。Agent-Runway skill directory 内では PowerShell の `"$(Get-Location)"` が検証済みです
- 通常のプロジェクトローカル利用では `ILH_DB_PATH` を未設定のままにします。そのプロジェクト用の特定 state file を意図的に使う場合だけ設定してください。runtime が `.agent-runway` を自動作成します
...
- Claude Code の `Stop` hook が設定されていない場合、Stop enforcement は advisory のままです
- receipt が証明するのは tool が実行されたことだけであり、タスク全体の意味的な正しさではありません
- secret や DB へアクセスできると、receipt の信頼性は下がります
- Claude Code hooks が設定されている場合、secret-key の読み取りはパス変種をまたいで拒否されます
- Claude Code hooks が設定されている場合、危険な shell command は実行前に確認を要求します
- Claude Code hooks が設定されている場合、gate 承認後に新しい receipt が追加されると、その承認は古くなり、`Stop` には新しい gate decision が必要になります
...
- Codex、Cursor はこのリポジトリでは MCP パスです; 追加で検証済みの host bridge がない限り shell/read/edit receipt の自動捕捉は主張しません
- Project Learning Ledger は現在のプロジェクトの ignored `.agent-runway/` 配下に置くローカルデータです。memory は evidence ではなく、preference は authorization ではありません
- Dynamic Context は `.agent-runway/dynamic-context.jsonl` に置くプロジェクトローカルの mission-scoped データです。1 record は 100k bytes までで、evidence や authorization ではありません
